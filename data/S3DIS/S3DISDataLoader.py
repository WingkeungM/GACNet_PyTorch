...
def pc_normalize(data, center):
    mindata = np.min(data[:, :3], axis=0)
    maxdata = np.max(data[:, :3], axis=0)
    center = (center - mindata) / (maxdata - mindata)
    data_xyz_norm = (data[:, :3] - mindata) / (maxdata - mindata)
    return data_xyz_norm, center
